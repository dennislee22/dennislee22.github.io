...
- Wildcard DNS entry must be configured; e.g. `*.apps.myhostname.com` to reduce Day-2 operational tasks to set separate DNS entry for each newly provisioned external-facing application/service.
